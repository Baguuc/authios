pub struct DetectedResult<T> {
    pub create: Vec<T>,
    pub delete: Vec<T>
}

pub fn detect_changes_in_vecs<T: PartialEq + Eq + std::hash::Hash + Clone>(old: Vec<T>, new: Vec<T>) -> DetectedResult<T> {
    ...
    return DetectedResult { create, delete };
}
